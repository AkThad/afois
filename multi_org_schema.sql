...
WHERE id != '00000000-0000-0000-0000-000000000000' LOOP -- Check if org already exists for this user to avoid duplicates if re-run
...
WHERE status IS NOT NULL
    AND status != 'POSSIBLE' ON CONFLICT DO NOTHING;
...
-- 6b. Create Company Documents Table
...
SELECT USING (id = ANY(get_my_org_ids()));
DROP POLICY IF EXISTS "Members can view org pipeline" ON public.pipeline_items;
CREATE POLICY "Members can view org pipeline" ON public.pipeline_items FOR ALL USING (org_id = ANY(get_my_org_ids()));
DROP POLICY IF EXISTS "Members can view org analysis" ON public.ai_analysis;
CREATE POLICY "Members can view org analysis" ON public.ai_analysis FOR ALL USING (org_id = ANY(get_my_org_ids()));
DROP POLICY IF EXISTS "Members can view org docs" ON public.company_documents;
CREATE POLICY "Members can view org docs" ON public.company_documents FOR ALL USING (org_id = ANY(get_my_org_ids()));
-- 11. Additional Policies
-- Allow users to see their own memberships
CREATE POLICY "Users can view own memberships" ON public.organization_members FOR
