...
create table public.user_profiles (
  ...
  target_naics text[],
  target_states text[],
  qualified_set_asides text[], -- e.g. ['8A', 'HUBZone', 'SDVOSB']
...
create table public.ai_analysis (
  ...
  pwin_score integer check (pwin_score >= 0 and pwin_score <= 100),
  recommendation text,
  summary text,
...
-- Policy: Opportunities are visible to authenticated users (for now, assuming single tenant or shared ops)
create policy "Authenticated users can view opportunities" on public.opportunities
  for select using (auth.role() = 'authenticated');

create policy "Authenticated users can view analysis" on public.ai_analysis
  for select using (auth.role() = 'authenticated');
